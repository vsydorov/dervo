...
def _load_yml_from_snake(
    ...
    for lvl, (fold, filenames) in enumerate(snake.items()):
        cfgs = {
            filename: yml_from_file(fold / filename)
...
def _perform_pyeval_updates(snake: Snake, pyeval_queries, yml_configs):
    """
    ...
    with vst.LogCaptorToRecords(pause="all") as lctr:
        exec(code, cfg_scope)
    ...
    if len(lctr.captured) > 0:
        ...
        for line in lctr.captured:
            captured_str += (
                mprefix(f"{line.name} {line.levelname}: " + line.getMessage(), ">> ")
                + "\n"
            )
        captured_str += "---- }}}} CAPTURED_PYEVAL"
...
def _yml_merge_summary(yml_merged_config, yml_configs, snake: Snake):
    ...
    for key, value in flat0.items():
        ...
        output += row_format.format(key, level, levelstars, value) + "\n"
    return output
