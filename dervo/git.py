...
def git_repo_perform_checkout_and_postcmd(
    repo, co_repo_fold, co_commit_sha, post_cmd, local_submodules, n_post_cmd_tries=2
):
    """
...
def git_repo_is_checkout_complete(co_repo_fold: Path, co_commit_sha: str):
    ...
    try:
        co_repo = git.Repo(str(co_repo_fold))
        assert co_repo.head.commit.hexsha == co_commit_sha, "commit shas must be same"
...
def git_repo_careful_checkout(
    repo, co_repo_fold, co_commit_sha, post_cmd, local_submodules, n_repo_checks=2
):
    """
    ...
    if not co_repo_fold.exists():
        git_repo_perform_checkout_and_postcmd(
            repo, co_repo_fold, co_commit_sha, post_cmd, local_submodules
        )
        ...
        for i in range(n_repo_checks):
            ...
            if co_good:
                ...
                break
            log.info(
                f"({i}) Waiting for checked out folder " f"to appear at {co_repo_fold}"
            )
            time.sleep(5)
        ...
        if not co_good:
            ...
            co_repo_fold = Path(
                tempfile.mkdtemp(
                    prefix=datetime_now, dir=str(co_repo_fold.parent), suffix="temp"
                )
            )
            git_repo_perform_checkout_and_postcmd(
                repo, co_repo_fold, co_commit_sha, post_cmd, local_submodules
            )
            log.info(f"Checked out code to alternative " f"location {co_repo_fold}")
    return co_repo_fold
...
def manage_code_checkout(
    ...
    assert co_commit_sha != RAWCOMMIT

    co_repo_basename = code_root.name
    assert checkout_folder is not None, "checkout_folder should be set"
    destination = Path(checkout_folder) / f"{co_repo_basename}/{co_commit_sha}"
    destination = git_repo_careful_checkout(
        repo, destination, co_commit_sha, post_cmd, local_submodules
    )
    return destination
